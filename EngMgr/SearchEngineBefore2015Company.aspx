<%@ Page Title="" Language="C#" MasterPageFile="~/SearchEngine.Master" AutoEventWireup="true" CodeBehind="SearchEngineBefore2015Company.aspx.cs" Inherits="EngMgr.SearchEngineBefore2015Company" EnableEventValidation="false" %>
...
<asp:Panel ID="SearchPnl" runat="server" Font-Bold="True" Font-Names="Tahoma" Font-Size="X-Large" Width="100%">
    ...
    <table class="auto-style11">
        ...
        <tr>
            <td>
                Equipment<br />
                <asp:SqlDataSource ID="companyds" runat="server" ConnectionString="<%$ ConnectionStrings:maincs %>" SelectCommand="SELECT DISTINCT [Customer] FROM [oldjobs]"></asp:SqlDataSource>
            </td>
            <td>
                Company<br />
                <ajaxToolkit:ComboBox ID="ComboBox1" runat="server" AutoPostBack="True" DataSourceID="companyds" DataTextField="Customer" DataValueField="Customer" MaxLength="0" style="display: inline;">
    ...
    <asp:SqlDataSource ID="eng2wadeds" runat="server" ConnectionString="<%$ ConnectionStrings:maincs %>" SelectCommand="SELECT [MTC Job No] AS MTC_Job_No, [Customer], [Equipment Type] AS Equipment_Type, [Serial Number] AS Serial_Number, [City/State] AS column1, [Ship Date] AS Ship_Date, [G A  Dwg] AS G_A_Dwg, [Install Dwg] AS Install_Dwg, [Sales Dwg] AS Sales_Dwg, [TA Dwg] AS TA_Dwg, [HY_PN Dwg] AS HY_PN_Dwg, [Elec Dwg] AS Elec_Dwg, [Hy Dwg] AS Hy_Dwg, [Pn Dwg] AS Pn_Dwg, [EL_HY Dwg] AS EL_HY_Dwg FROM [oldjobs] WHERE ([Customer] LIKE '%' + @Customer + '%')">
        <SelectParameters>
            <asp:ControlParameter ControlID="ComboBox1" Name="Customer" PropertyName="SelectedValue" Type="String" />
        </SelectParameters>
    </asp:SqlDataSource>
    <asp:ScriptManager ID="ScriptManagerecompanywade" runat="server">
    </asp:ScriptManager>
