<%@ Page Title="" Language="C#" MasterPageFile="~/Engineer.Master" AutoEventWireup="true" CodeBehind="EngineerRG.aspx.cs" Inherits="EngMgr.EngineerRG" EnableEventValidation="false"%>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <asp:SqlDataSource ID="RGDS" runat="server" ConnectionString="<%$ ConnectionStrings:maincs %>" SelectCommand="SELECT [ID], [MECHNotes], [Company], [COnumber], [Item], [DateRecvd], [ShipDate], [GAReqdDate], [GACompDate], [FirstAppDate], [TACompDAte], [TARELtoPlanDate], [RA], [RG], [TL], [BW], [JT], [LW], [HC], [RC], [RW], [WW], [ELREQD] FROM [JobEngine] WHERE (([RG] = @RG) AND ([Status] = @Status) AND ([TARELtoPlanDate] IS NULL)) ORDER BY [ShipDate], [Company], [COnumber]" UpdateCommand="UPDATE JobEngine SET MECHNotes = @MECHNotes, RA = @RA, RG = @RG, TL = @TL, BW = @BW, JT = @JT, LW = @LW, HC = @HC, RC = @RC, RW = @RW, WW = @WW WHERE (ID = @ID)">
        <SelectParameters>
            <asp:QueryStringParameter DefaultValue="True" Name="RG" QueryStringField="True" />
    ...
    <asp:Panel ID="Panel1" runat="server" HorizontalAlign="Left" Width="50%">
     ...
     <asp:GridView ID="RAGridView" runat="server" AutoGenerateColumns="False" CaptionAlign="Left" DataKeyNames="ID" DataSourceID="RGDS" CellPadding="4" ForeColor="#333333">
